...
def _group(name: str) -> str:
    return name.split(":", 1)[0].strip() if ":" in name else "general"
...
total = len(results)
print(f"Eval harness: {passed}/{total} checks passed\n")
...
if "--write" in sys.argv:
    ...
    for name, ok, detail in results:
        groups.setdefault(_group(name), []).append((name, ok, detail))
    ...
    payload = {
        ...
        "generated_at": ts, "commit": sha,
        "summary": {"passed": passed, "total": total, "all_green": passed == total},
        "by_group": {g: {"passed": sum(1 for _, ok, _ in v if ok), "total": len(v)}
                     for g, v in groups.items()},
        "checks": [{"group": _group(n), "name": n, "passed": ok,
                    "detail": None if ok else str(d)} for n, ok, d in results],
    }
    (ROOT / "evals" / "results.json").write_text(json.dumps(payload, indent=2) + "\n")

    lines = [
        "# Eval results",
        "",
        "> Auto-generated by `python3 evals/run_evals.py --write` (refreshed by CI on every push to "
        "`main`). Do not edit by hand.",
        "",
        ...
        "against real vendor exports during development (see each skill's parsing reference); the "
        "committed fixtures make the check self-contained and repeatable.",
        "",
        "## By source",
        "",
        "| Source | Passing |",
    ...
    for g in sorted(groups):
        gp = sum(1 for _, ok, _ in groups[g] if ok)
        lines.append(f"| {g} | {gp}/{len(groups[g])} |")
    lines += ["", "## Checks", ""]
    for g in sorted(groups):
        lines.append(f"### {g}")
        lines.append("")
    ...
    print(f"\nWrote evals/RESULTS.md and evals/results.json ({passed}/{total}).")
