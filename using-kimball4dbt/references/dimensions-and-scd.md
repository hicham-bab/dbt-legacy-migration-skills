...
day-of-week, fiscal periods, holiday flags). Prefer the **`dbt_date`** package
(`godatadriven/dbt_date`) — `{{ dbt_date.get_date_dimension('2015-01-01', '2035-12-31') }}` builds a
full calendar dimension with fiscal-period helpers; otherwise use `dbt_utils.date_spine` or a seed.
Every date FK on every fact references it (`order_date_key`, `ship_date_key`, …). See foundations →
